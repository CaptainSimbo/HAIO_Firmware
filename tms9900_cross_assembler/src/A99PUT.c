	...
	switch (hexflg)
	{
		...
		case PUTCODE:	bptr =binbuf;
				for (count = 1; count <= nbytes; count++)
				{
					...
					if (++hxbytes == reclen)  flshhbf(pc+count);
				}
...
void puthex2(unsigned char number, char **lineptr)
{
	...
	if ((*++*lineptr = (number & 0x0f) + '0') > '9') **lineptr += 7;
	++(*lineptr);
}
...
void putchr(char byte, struct diskbuf *dskbuf)
{
	...
	if(kbhit())
	{
		c = getchar();
		if(c == CTLC) wipeout("\n");
		if(c == CTLS) while(kbhit() == 0) c= getchar();
	}
	switch (dskbuf -> fd)
	{
...
void flush(struct diskbuf *dskbuf)
{
	...
	while (dskbuf -> pointr < &dskbuf -> space[128]) *(dskbuf -> pointr)++ = 0;
