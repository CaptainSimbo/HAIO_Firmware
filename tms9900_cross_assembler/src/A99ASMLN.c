	...
	switch (opcode)
	{
		...
		case 1:	if (label[0] == '\0')		/* EQU.	*/
			  ...
			  /* MSB set for multiple declarations */
				...
				if (address != t) markerr('M');  /* Lable defined multiple times */
				goto chkargs;
					...
					while ((c = getitem(&t,SMALST)) != END_LIN)	{
						switch (c) {
							...
							case VALUE:
								...
								cflag = TRUE;
								t=eval(START);
								if ((evalerr || t > 0x00ff) && (t < 0xff80)) markerr('V');
								*bptr++ = t;
								nbytes += 1;
								break;
     ...
     if (getitem(&t,SMALST) != END_LIN) markerr('T');
				return;
...
{
	unsigned short regmask,regval,t,value,operand,c;
	char disp;
	char d;
