...

{
	...
	switch (getchr(&c,SKIP))
	{
		case ALPHA:	backchr;
				getname(temp);
				if (bbsearch(temp,value,attrib,0,numopcs(),getopc)) return 1;
				else
				{
				 	strcpy(dxop,temp);
					strcat(dxop,PADDING);
...
int bbsearch(char *name, unsigned *value, unsigned *attrib, int low, int high, void (*table)())
{
	char temp[5];
	char *pntr1, *pntr2;
	int t, check;
	...
	if (t == 0) return 1;
	if (t > 0) return bbsearch(name,value,attrib,check+1,high,table);
...
{
	char c, c1, *tmpptr, temp[2*SYMLEN];
		...
		case ALPHA:	backchr;
				...
				if (slookup(temp) == 0)
				{
					*value = sympoint -> symvalu;
					if ((unsigned char)(sympoint -> symname[0]) > 0x7F) directok = FALSE;
				}
				else
				{
					evalerr = TRUE;
